# memgraph — Prototype Handoff Document
...
Working name: **memgraph** (placeholder).
...
```
memgraph assert   --subject … --predicate … --object … --class observation --scope module:auth
memgraph facts    --entity AuthService --as-of 2026-03-01
memgraph neighbor --entity AuthService --depth 2
memgraph search   "redis migration"
memgraph history  --subject AuthService --predicate depends_on
memgraph ingest   --episode <id> --file facts.jsonl
memgraph decay
...
- **Version pinning.** Not every Datalevin release is in the pod registry; pin
  ...
  pinned `dtlv` binary from `$PATH` / `$MEMGRAPH_DTLV`.)
