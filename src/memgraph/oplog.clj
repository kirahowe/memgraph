(ns memgraph.oplog
  ...
  3. Convergence through surfacing, not through merge magic. A CRDT forces
     agreement by construction; memgraph wants disagreement made visible.
  ...
  The write lease (memgraph.lease) still serializes writers on ONE machine;
...
(defn writer-id!
  "A stable id for this machine/agent: $MEMGRAPH_WRITER, or a uuid minted
  once and kept in <db>.oplog/writer."
  [db]
  (or (System/getenv "MEMGRAPH_WRITER")
...
(defn- remap-entity!
  ...
  (let [foreign-id (:id ent)]
    (or (some->> (get @emap (keyword foreign-id))
                 (hash-map :id))
        (let [ensure (requiring-resolve 'memgraph.core/ensure-entity)
  ...
  (let [own (writer-id! db)
    ...
    {:status :reconciled
     ...
     :hint (when (pos? (count candidates))
             "run `memgraph judge --sweep` (or consolidate) to judge what the writers couldn't see")}))
